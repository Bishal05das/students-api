...
func (s *Sqlite) GetStudentById(id int64) (types.Student, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return types.Student{}, fmt.Errorf("no student found with id %s",fmt.Sprint(id)) // No student found
		}
		return types.Student{}, fmt.Errorf("query error: %w",err) // Other error
	}
	return student, nil
}
